...
export default function Header({ onOpenDemo, onOpenSettings, isOnline, onRefresh, isRefreshing }) {
    ...
    <header className="sticky top-0 z-40 w-full glass-panel border-b border-slate-800/80 bg-cyber-950/85 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-2 px-3 py-1.5 rounded-full bg-cyber-900/90 border border-slate-800 text-xs">
            <span className={`w-2 h-2 rounded-full ${isOnline ? 'bg-emerald-400 animate-pulse' : 'bg-rose-500'}`} />
            <span className="text-slate-300 font-mono">
              API: {isOnline ? 'ONLINE' : 'OFFLINE'}
            </span>
            <span className="text-slate-600">|</span>
            <span className="text-slate-400 font-mono truncate max-w-[140px]" title={currentKey}>
              Key: {currentKey ? `${currentKey.slice(0, 10)}...` : 'None'}
            </span>
          </div>
